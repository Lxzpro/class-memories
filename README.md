...
- 管理员概览、可重试批量上传、成员/隐私审核、全媒体故事与权限和操作记录
- Supabase PostgreSQL 迁移、RLS 和待审核成员权限隔离
...
Next.js App Router（Vercel）
  ...
  ├─ Supabase PostgreSQL：照片资料、成员、权限、审核状态、留言
...
4. 建议在正式开放注册前配置自己的 SMTP；免费默认邮件服务只适合开发测试。
...
- 新注册成员默认进入 pending 状态，只有管理员审核通过后才能进入相册
- 成员、照片、人物关联、指定访问、收藏、隐私申请和留言的 RLS
- 匿名用户无法读取照片表
- 普通成员无法把自己提升为管理员

旧迁移中已有的邀请码表和兑换记录会作为历史结构保留，当前应用不再读取或写入它们。无需执行删除迁移，也不会影响已有成员或照片数据。
