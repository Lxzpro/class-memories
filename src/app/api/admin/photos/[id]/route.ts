...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!DEMO_MODE) {
    ...
    if (input.peopleIds) {
      ...
      if (input.peopleIds.length) { const { data: preferences } = await supabase.from("profiles").select("id,require_tag_approval").in("id", input.peopleIds); await supabase.from("photo_people").insert(input.peopleIds.map((userId) => ({ photo_id: id, user_id: userId, consent_status: preferences?.find((profile) => profile.id === userId)?.require_tag_approval ? "pending" : "approved" }))); }
    }
