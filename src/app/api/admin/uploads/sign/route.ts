...
export async function POST(request: Request) {
  ...
  const parsed = memberUploadSignSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return Response.json({ error: "支持 25MB 内的图片，或 200MB 内的 MP4、WebM 视频。" }, { status: 400 });
