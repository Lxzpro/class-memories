...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const authorName = getPublicProfileName(user);
  if (DEMO_MODE) return Response.json({ comment: { id: `demo-${Date.now()}`, photoId: id, userId: user.id, authorName, content, status: "visible", createdAt: new Date().toISOString() } });
  ...
  return Response.json({ comment: { id: data.id, photoId: id, userId: user.id, authorName, content, status: "visible", createdAt: data.created_at } });
}
