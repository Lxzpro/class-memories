...
export interface AdminDashboardData { photos: Photo[]; members: Profile[]; logs: AdminLogView[]; privacyRequests: PrivacyRequest[] }
...
export async function getAdminDashboardData(): Promise<AdminDashboardData> {
  if (DEMO_MODE) return {
    ...
    logs: [
      ...
      { id: "log-3", action: "更新了照片可见范围", resourceType: "照片", createdAt: "2026-08-23T08:40:00.000Z", adminName: "林老师" },
    ],
    privacyRequests: [
  ...
  const [{ data: photoRows }, { data: memberRows }, { data: logRows }, { data: privacyRows }] = await Promise.all([
  ...
  return { photos, members, logs, privacyRequests };
}
