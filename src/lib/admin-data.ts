...
export async function getAdminDashboardData(): Promise<AdminDashboardData> {
  ...
  const [{ data: photoRows }, { data: memberRows }, { data: inviteRows }, { data: logRows }, { data: privacyRows }] = await Promise.all([
    ...
    supabase.from("invite_codes").select("id,code_ciphertext,expires_at,max_uses,used_count,revoked_at,created_at,invite_redemptions(redeemed_at,status,profiles(display_name))").order("created_at", { ascending: false }),
