...
export async function getAdminDashboardData(): Promise<AdminDashboardData> {
  ...
  const [{ data: photoRows }, { data: memberRows }, { data: inviteRows }, { data: logRows }, { data: privacyRows }] = await Promise.all([
    supabase.from("photos").select("*").neq("review_status", "deleted").order("created_at", { ascending: false }),
  ...
  const photos = await Promise.all((photoRows ?? []).map(async (row): Promise<Photo> => ({
    id: String(row.id), title: String(row.title), description: String(row.description ?? ""), originalKey: String(row.original_key), previewKey: String(row.preview_key), thumbnailKey: String(row.thumbnail_key),
    previewUrl: await storage.createReadUrl({ key: String(row.preview_key) }), thumbnailUrl: await storage.createReadUrl({ key: String(row.thumbnail_key) }), width: Number(row.width), height: Number(row.height), location: String(row.location ?? ""), people: [], tags: [], visibility: row.visibility, selectedUserIds: [], downloadAllowed: Boolean(row.download_allowed), reviewStatus: row.review_status, uploadedBy: String(row.uploaded_by), createdAt: String(row.created_at),
  })));
  const members: Profile[] = (memberRows ?? []).map((row) => ({ id: String(row.id), email: String(row.email), displayName: String(row.display_name), avatarKey: row.avatar_key ? String(row.avatar_key) : null, role: row.role, status: row.status, showRealName: Boolean(row.show_real_name), requireTagApproval: Boolean(row.require_tag_approval), allowOriginalDownload: Boolean(row.allow_original_download), createdAt: String(row.created_at) }));
