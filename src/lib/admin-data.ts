...
export interface AdminInviteView { id: string; codeAvailable: boolean; expiresAt: string; maxUses: number; usedCount: number; revokedAt: string | null; createdAt: string; redemptions: Array<{ name: string; redeemedAt: string }> }
...
type AdminNameRow = { display_name?: unknown; real_name?: unknown } | null;

function adminName(row: AdminNameRow, fallback = "班级成员") {
  if (!row) return fallback;
  const nickname = typeof row.display_name === "string" ? row.display_name : "";
  const realName = typeof row.real_name === "string" ? row.real_name.trim() : "";
  if (!realName || realName === nickname) return nickname || fallback;
  return `${realName}（昵称：${nickname}）`;
}
...
export async function getAdminDashboardData(): Promise<AdminDashboardData> {
  if (DEMO_MODE) return {
    ...
    invites: [
      { id: "invite-active", codeAvailable: Boolean(MOCK_INVITE_CODES["invite-active"]), expiresAt: "2026-09-10T23:59:59.000Z", maxUses: 50, usedCount: 8, revokedAt: null, createdAt: "2026-08-18T08:00:00.000Z", redemptions: [{ name: "夏宁", redeemedAt: "2026-08-20T09:10:00.000Z" }, { name: "周予安", redeemedAt: "2026-08-21T12:30:00.000Z" }] },
      { id: "invite-revoked", codeAvailable: Boolean(MOCK_INVITE_CODES["invite-revoked"]), expiresAt: "2026-09-01T23:59:59.000Z", maxUses: 10, usedCount: 3, revokedAt: "2026-08-23T10:00:00.000Z", createdAt: "2026-08-10T08:00:00.000Z", redemptions: [{ name: "林澈", redeemedAt: "2026-08-12T10:00:00.000Z" }] },
    ],
    logs: [
  ...
  const [{ data: photoRows }, { data: memberRows }, { data: inviteRows }, { data: logRows }, { data: privacyRows }] = await Promise.all([
    ...
    supabase.from("invite_codes").select("id,code_ciphertext,expires_at,max_uses,used_count,revoked_at,created_at,invite_redemptions(redeemed_at,status,profiles(display_name,real_name))").order("created_at", { ascending: false }),
    supabase.from("admin_logs").select("id,action,resource_type,created_at,profiles(display_name,real_name)").order("created_at", { ascending: false }).limit(30),
    supabase.from("privacy_requests").select("id,user_id,photo_id,kind,message,status,created_at,resolved_at,profiles(display_name,real_name),photos(title)").order("created_at", { ascending: false }),
  ]);
  const storage = getStorageAdapter();
  const memberNames = new Map((memberRows ?? []).map((row) => [String(row.id), adminName(row)]));
  ...
  const members: Profile[] = (memberRows ?? []).map((row) => ({ id: String(row.id), email: String(row.email), displayName: String(row.display_name), realName: typeof row.real_name === "string" ? row.real_name : null, avatarKey: row.avatar_key ? String(row.avatar_key) : null, role: row.role, status: row.status, showRealName: Boolean(row.show_real_name), allowOriginalDownload: Boolean(row.allow_original_download), createdAt: String(row.created_at) }));
  const invites: AdminInviteView[] = (inviteRows ?? []).map((row) => ({ id: String(row.id), codeAvailable: typeof row.code_ciphertext === "string" && row.code_ciphertext.length > 0, expiresAt: String(row.expires_at), maxUses: Number(row.max_uses), usedCount: Number(row.used_count), revokedAt: row.revoked_at ? String(row.revoked_at) : null, createdAt: String(row.created_at), redemptions: ((row.invite_redemptions ?? []) as unknown as Array<{ redeemed_at: string; profiles?: AdminNameRow }>).map((item) => ({ name: adminName(item.profiles ?? null), redeemedAt: item.redeemed_at })) }));
  const logs: AdminLogView[] = (logRows ?? []).map((row) => ({ id: String(row.id), action: String(row.action), resourceType: String(row.resource_type), createdAt: String(row.created_at), adminName: adminName(row.profiles as unknown as AdminNameRow, "管理员") }));
  const privacyRequests: PrivacyRequest[] = (privacyRows ?? []).map((row) => ({
    id: String(row.id), userId: String(row.user_id), userName: adminName(row.profiles as unknown as AdminNameRow),
